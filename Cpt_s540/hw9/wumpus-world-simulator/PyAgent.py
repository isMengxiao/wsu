...
class Agent:
    ...
    def Initialize(self):
        ...
        if (self.firstTry):
            ...
            if (self.worldState.goldLocation == [0, 0]):
                # Didn't find gold on first try (should never
    ...
    def UpdateState(self, percept):
        ...
        if (self.previousAction == Action.GOFORWARD):
            ...
            if (percept['Bump']):
                ...
                if (self.worldState.goldLocation == [0, 0]):
                    # If haven't found gold yet, add this location
                    # to the pathToGold
        ...
        if (self.previousAction == Action.SHOOT):
            ...
            # facing RIGHT, perceive a stench, and don't know the
            # wumpus location.
            # In this case, a SCREAM percept means the wumpus is in (2,1)
            # (and dead),
    ...
    # HW5
    #
    # Update possible wumpus locations based on the current set of
    # stench locations and clear locations.
    #

    def UpdatePossibleWumpusLocations(self):
        self.possibleWumpusLocations = []
        # Determine possible wumpus locations consistent with
        # available stench information
        for location1 in self.stenchLocations:
            ...
            if (not self.possibleWumpusLocations):
                # Must be first stench location in list, so add all adjacent
                # locations
    ...
    def ChooseAction(self, percept):
        ...
        if (percept['Stench'] and (self.worldState.agentLocation == [1, 1]) and
            ...
            action = Action.SHOOT
        elif ((forwardLocation in self.safeLocations) and
              (forwardLocation not in self.visitedLocations)):
            # If happen to be facing safe unvisited location, then move there
            action = Action.GOFORWARD
        else:
            # Choose randomly from GOFORWARD, TURNLEFT, and TURNRIGHT,
            # but don't
    ...
    def AddActionsFromPath(self, forward):
        ...
        for nextLocation in path[1:]:
            ...
            # Find shortest turn sequence (assuming RIGHT=0,
            # UP=1, LEFT=2, DOWN=3)
    ...
    def AddAdjacentLocations(self, locationList, location):
        ...
        if ((worldSize == 0) or (location[1] < worldSize)):
            self.AddNewLocation(locationList, [location[0], location[1] + 1])
            # up
        if ((worldSize == 0) or (location[0] < worldSize)):
            self.AddNewLocation(locationList, [location[0] + 1, location[1]])
            # right
        if (location[0] > 1):
            self.AddNewLocation(locationList, [location[0] - 1, location[1]])
            # left
    ...
    # HW5
    #
    # Filters from safeLocations any locations that are outside the upper
    # or right borders of the world.
    #
