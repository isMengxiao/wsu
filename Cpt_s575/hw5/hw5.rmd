...
for (section in sections){
    sums = 0
    for (page in 1:10){
        data = fromJSON(paste("http://content.guardianapis.com/search?section=", section, "&show-fields=wordcount%2Cbody&page=", page, "&page-size=120&api-key=14453543-e613-4acb-a757-e79000c47ca9", sep=""))$response$results 
        for (i in 1:120){
            if (data[[i]]["type"]!="article"){
                sums = sums+1
            }
        
            mydata[s*1000+(page-1)*100+i,] <- c(data[[i]]$id, data[[i]]$webTitle, data[[i]]$fields[[1]], data[[i]]$fields[[2]],  data[[i]]$sectionId)

            Encoding(mydata[s*1000+(page-1)*100+i,][["body"]]) <- "UTF-8"
            Encoding(mydata[s*1000+(page-1)*100+i,][["title"]]) <- "UTF-8"
        
            mydata[s*1000+(page-1)*100+i,]$body <- gsub("<.*?>[^[:alnum:][:space:]]", "", mydata[s*1000+(page-1)*100+i,][["body"]])
        #    mydata[s*1000+(page-1)*100+i,]$body <- gsub("[^[:alnum:][:space:]]","", mydata[s*1000+(page-1)*100+i,][["body"]])
            mydata[s*1000+(page-1)*100+i,]$body <- tolower(mydata[s*1000+(page-1)*100+i,][["body"]])
        }
    }
    s = s+1
}
