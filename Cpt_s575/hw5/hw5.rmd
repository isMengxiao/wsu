...
cor_terms = sort(findCorrelation(cor_matrix, cutoff = 0.85))
TermMatrix = TermMatrix[, ~c(cor_terms)]

TM_train = TermMatrix[1:floor(nrow(TermMatrix)),]
TM_test = TermMatrix[floor(nrow(TermMatrix))+1,]
data_train = as.facotr(mydata[1:floot(nrow(mydata)),])
data_test = as.facotr(mydata[floor(nrow(mydata)+1:),])
